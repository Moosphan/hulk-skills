...
from tts_support import TTSConfig, edge_tts_available
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tts-language", default="auto", choices=["auto", "zh", "en", "bilingual"], help="Speech language for TTS. auto follows the current session language.")
    ...
    parser.add_argument("--persona-voice-overrides", default="", help="Comma-separated persona=voice overrides for persona-aware prompt synthesis compatibility.")
...
def main() -> int:
    ...
    if args.mode == "simulate":
        intro_question = build_intro_question(args.level, planned_language_for_round(interview_plan, "intro", args.language))
    ...
    def spoken_prompt_for_question(question, runtime_language: str) -> str:
        target_language = speech_language_for(runtime_language, question.question)
        spoken_question_map = getattr(question, "spoken_question", {}) or {}
        spoken_question = str(spoken_question_map.get(target_language, "")).strip() if isinstance(spoken_question_map, dict) else ""
        return spoken_question or question.question
        ...
        try:
            results.append(
                evaluate_question_batch(
                    ...
                    spoken_question=spoken_prompt_for_question(question, planned_language_for_round(interview_plan, question.round, args.language)),
                )
            )
    ...
    if args.enable_tts:
        if edge_tts_available():
            tts_files = synthesize_summary_artifacts(
                ...
                results,
                decision,
                tts_config.resolved_voice(args.language),
